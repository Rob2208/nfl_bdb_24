...
for(i in 1:(nrow(pred)-7)){
  ...
  points(tracking_play$x_bc[i], tracking_play$y_bc[i], pch = 2, lwd = 6, col = coloff)
  ...
  abline(v = pbp_info$yardline_100, col = collos, lty = 2, lwd = 2)
  abline(v = 0, lwd = 1.5) # endzone
  abline(v = max(tracking_play$x_bc[i] - mean(pred[i,]),0), col = 1, lwd = 2)
  abline(v = max(tracking_play$x_bc[i] - mean(pred[i,]),0), col = coldens, lwd = 2)
  abline(v = pbp_info$true_yardline, col = coleop, lty = 2, lwd = 2)
...
vis_tackle_plays3 <- function(gpId,
  ...
  abline(v = 1:20 *5, col = "lightgray", lwd = 0.5)
  abline(v = tackle_info$yardline_100, col = collos, lty = 2, lwd = 2)
  abline(v = 0, lwd = 1.5) # endzone
  abline(v = tackle_frame$x_bc - mean(pred_r), col = 1, lwd = 2)
  abline(v = tackle_frame$x_bc - mean(pred_r), col = coldens, lwd = 2)
  abline(v = tackle_info$true_yardline, col = coleop, lty = 2, lwd = 2)
  ...
  abline(v = tackle_frame$x_bc - mean(pred_s), col = coldens, lwd = 2)
  abline(v = tackle_info$true_yardline, col = coleop, lty = 2, lwd = 2)
...
The below Figure \ref{fig:overview} summarizes the above steps. 

```{r ssa-logo, fig.align = 'center', fig.cap = "\\label{fig:overview} Graphical overview of the steps of our analysis.", warning=FALSE, message=FALSE}
...
make_pal <- function(value,col_low = "red",col_high = "darkgreen",nq = 10){
...
tbl <- reactable(crosstalk_data,
  ...
  columns = list(
    ...
    sum_tackle_gains = colDef(
      ...
      style = color_scales(best_tacklers, colors = make_pal(best_tacklers$sum_tackle_gains[best_tacklers$n_tackles >= 10])),
      maxWidth = 80
    ),
    sum_tackle_gains2 = colDef(
      name = "PEP[2]",
      format = colFormat(digits = 2),
      style = color_scales(best_tacklers, colors = make_pal(best_tacklers$sum_tackle_gains2[best_tacklers$n_tackles >= 10])),
      maxWidth = 80
    ),
    avg_per_tackle = colDef(
      name = "Avg PEP[1]/Tackle",
      format = colFormat(digits = 2),
      style = color_scales(best_tacklers, colors = make_pal(best_tacklers$avg_per_tackle[best_tacklers$n_tackles >= 10])),
      maxWidth = 80
    ),
    avg_per_tackle2 = colDef(
      name = "Avg PEP[2]/Tackle",
      format = colFormat(digits = 2),
      style = color_scales(best_tacklers, colors = make_pal(best_tacklers$avg_per_tackle2[best_tacklers$n_tackles >= 10])),
      maxWidth = 80
    )
  )
...
```
Rouven analysiert die Ergebnisse :-)
