...
tackle_list <- readRDS("../data/tackles_info_data.rds")
...
rt_tbl <- reactable(tackle_info_real2 %>% 
                      select(-gameplayId,-uId,-absoluteYardlineNumber),
                    bordered = TRUE,
...
pos_df <- best_tacklers %>% group_by(position) %>% summarise(avg = mean(sum_tackle_gains),avg2 = mean(sum_tackle_gains2),avg_n_tackles = mean(n_tackles)) %>% arrange(desc(avg))
